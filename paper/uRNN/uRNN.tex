...
\subsubsection{Gradient propagation}
As discussed in section 2, one of the most important features of learning long term dependencies is controlling $\frac{\partial C}{\partial h_t}$. In order to study how the different models propagate this quantity, we examined the norms of this gradients as a function of $t$. To do this, we run the models for the adding problem and calculate the gradients of the cost with respect to the hidden units at the beginning of training and after 100 iterations
...
We see in figure 4 the results. At first sight we see that during the begining of training, the uRNN propagates gradients perfectly, while the other models have exponentially vanishing gradients. This makes perfect sense, because since the biases of the uRNN are initialized to 0, the network starts as a linear network with a unitary weight matrix. If we look at the equations of section 2, this implies that the jacobian of the nonlinearity is the identity, and to get $\frac{\partial C}{\partial h_t}$ from $\frac{\partial C}{\partial h_T}$ we just multiply by unitary matrices, therefore preserving norm exactly. After 100 iterations of training, all networks start to get vanishing gradients problems, although the uRNN still propagates this information better than the rest.

\subsubsection{Hidden state saturation}

If a network needs to make predictions based on information spanning many time steps, then it necessary for it to incorporate new information for a very long time. In order to test the capacity of the network to keep incorporating new memories, we test the saturation of the hidden states. What this means is that if the hidden states of an RNN become constant after a certain amount of iterations, then it is clearly not going to change it's prediction based on this new information. The metric we designed to study this phenomenon is the euclidean distance between the hidden state at time $t$ and the final hidden state at time $T$. If $d(h_t, h_T)$ goes to zero quickly, then the information passed after that point wasn't incorporated by the network.
...
Theano~\cite{Fred2010} for % are you guys using blocks? nope :P
